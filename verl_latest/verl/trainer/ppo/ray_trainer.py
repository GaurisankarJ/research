...
class RayPPOTrainer:
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            skip_special = self.config.trainer.get("rollout_prompt_log_skip_special_tokens", True)
            inputs = self.tokenizer.batch_decode(batch.batch["prompts"], skip_special_tokens=skip_special)
            ...
            if "tool_call_counts" in batch.non_tensor_batch:
                tcc = batch.non_tensor_batch["tool_call_counts"]
                reward_extra_infos_to_dump["num_search_calls"] = (
                    tcc.tolist() if hasattr(tcc, "tolist") else list(tcc)
                )
            if "__num_turns__" in batch.non_tensor_batch:
                nt = batch.non_tensor_batch["__num_turns__"]
                reward_extra_infos_to_dump["num_turns"] = nt.tolist() if hasattr(nt, "tolist") else list(nt)
            if "raw_prompt" in batch.non_tensor_batch:
                rp = batch.non_tensor_batch["raw_prompt"]
                reward_extra_infos_to_dump["raw_prompt"] = rp.tolist() if hasattr(rp, "tolist") else list(rp)
            for _re_key in (
                "re_search_termination_reason",
                "re_search_response_digest",
                "re_search_response_token_count",
                "re_search_budget_tokens",
                "re_search_search_segments",
            ):
                if _re_key in batch.non_tensor_batch:
                    _arr = batch.non_tensor_batch[_re_key]
                    reward_extra_infos_to_dump[_re_key] = (
                        _arr.tolist() if hasattr(_arr, "tolist") else list(_arr)
                    )
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # ReSearch agent: rollout JSONL also logs ``re_search_termination_reason``; here we mirror scalars for W&B.
                if "re_search_response_token_count" in batch.non_tensor_batch:
                    _rtc = np.asarray(batch.non_tensor_batch["re_search_response_token_count"], dtype=np.float64)
                    metrics["re_search/response_token_count_mean"] = float(np.mean(_rtc))
                    _bud = batch.non_tensor_batch.get("re_search_budget_tokens")
                    if _bud is not None:
                        _b = float(np.asarray(_bud, dtype=np.float64).reshape(-1)[0])
                        metrics["re_search/budget_tokens"] = _b
                        if _b > 0:
                            metrics["re_search/budget_utilization"] = float(np.mean(_rtc / _b))
                if "re_search_termination_reason" in batch.non_tensor_batch:
                    _tr = batch.non_tensor_batch["re_search_termination_reason"]
                    _reasons = _tr.tolist() if hasattr(_tr, "tolist") else list(_tr)
                    metrics["re_search/termination_reason"] = _reasons[0] if len(_reasons) == 1 else str(_reasons)
