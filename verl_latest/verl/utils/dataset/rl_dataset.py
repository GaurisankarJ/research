...
from verl.utils.dataset.re_search_templates import QWEN_CHAT_IM_END
...
class RLHFDataset(Dataset):
    ...
    ReSearch / legacy prompt construction (optional, text-only):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                if self.prompt_template_name is not None:
                    raise ValueError(
                        "data.prompt_template_name is set but a multimodal processor is configured; "
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if self.prompt_template_name is not None:
                            messages = self._re_search_messages_for_doc(doc)
                            if self.re_search_use_chat_format and self.re_search_add_qwen_chat:
                                from verl.utils.dataset.re_search_templates import build_qwen_manual_chat_prompt

                                text = build_qwen_manual_chat_prompt(
                                    messages,
                                    add_thinking=self.re_search_add_thinking,
                                    im_end=self.re_search_qwen_im_end,
                                )
                                tokenized_prompt = tokenizer.encode(text, add_special_tokens=False)
                            elif self.re_search_use_chat_format:
                                ...
                                tokenized_prompt = tokenizer.encode(
                                    text, add_special_tokens=self.re_search_plain_add_special_tokens
                                )
                        else:
                            tokenized_prompt = tokenizer.apply_chat_template(
    ...
    def _re_search_messages_for_doc(self, doc: dict) -> list[dict]:
        """Build chat messages from a ReSearch template + string column (legacy ``RLHFDataset``).
        ...
        * ``data.prompt_template_name`` (Python dict in ``re_search_templates``): ``template.format(prompt=question)``,
        ...
        raw = doc[self.prompt_key]
        if isinstance(raw, list):
            raise TypeError(
                f"prompt_template_name is set but {self.prompt_key!r} contains a message list; "
                "omit prompt_template_name for pre-formatted chat data."
            )
        question = str(raw)
        if self.re_search_use_chat_format:
            return [
    ...
    def __getitem__(self, item):
        ...
        if self.prompt_template_name is not None:
            row_dict["raw_prompt"] = self._re_search_messages_for_doc(row_dict)
        else:
