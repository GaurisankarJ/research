...
class RolloutConfig(BaseConfig):
    ...
    # ReSearch / Search-R1 style: HTTP retriever used by ``re_search_agent`` agent loop (legacy ``vLLMRolloutWithSearch``).
    # POST ``{search_url}/batch_search`` with JSON ``{"query": [...], "top_n": n}``; response format must match legacy.
