#!/usr/bin/env bash
# GRPO + vLLM — Qwen3-0.6B-Instruct, **2× A100-80GB** (~80GB each): colocated FSDP + vLLM + re_search per node.
...
# | 4× GPU, TP=2 rollout | 2× GPU, ``tensor_model_parallel_size=1`` |
# | ``train_batch_size=256`` (split across GPUs) | ``TRAIN_BATCH_SIZE=16`` (global; raise env until OOM); optional ``GEN_BATCH_SIZE`` |
...
# | ``ref`` ``param_offload=True`` | ``param_offload=False`` (2× GPU; set True if actor OOM) |
...
# 2 GPUs per node (override e.g. CUDA_VISIBLE_DEVICES=2,3 if needed)
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}"

# Hydra: full Python tracebacks (default is abbreviated)
export HYDRA_FULL_ERROR="${HYDRA_FULL_ERROR:-1}"

# Colocated FSDP actor + ref + vLLM: reduce allocator fragmentation during weight sync
...
trainer_experiment_name="${WANDB_EXPERIMENT_NAME:-qwen3_0.6b_instruct_grpo_gpu_2_80gb}"
...
# Training batch: global batch across 2×80GB; lower if actor/ref OOM. ``GEN_BATCH_SIZE`` can exceed train batch.
# ``gen_batch_size × rollout_n`` must divide ``ROLLOUT_AGENT_NUM_WORKERS`` (default ``rollout_n``); e.g. 16×5=80 with workers=5.
...
vllm_gpu_mem_util="${VLLM_GPU_MEM_UTIL:-0.82}"

vllm_max_model_len="${VLLM_MAX_MODEL_LEN:-9216}"
max_prompt_length="${MAX_PROMPT_LENGTH:-512}"
max_response_length="${MAX_RESPONSE_LENGTH:-8192}"

max_token_len_per_gpu="${MAX_TOKEN_LEN_PER_GPU:-20480}"
max_rollout_logprob_token_len_per_gpu="${MAX_ROLLOUT_LOGPROB_TOKEN_LEN_PER_GPU:-20480}"
max_ref_logprob_token_len_per_gpu="${MAX_REF_LOGPROB_TOKEN_LEN_PER_GPU:-20480}"

vllm_max_num_seqs="${VLLM_MAX_NUM_SEQS:-10}"
rollout_n="${ROLLOUT_N:-5}"
rollout_max_num_batched_tokens="${ROLLOUT_MAX_NUM_BATCHED_TOKENS:-20480}"
