...
max_rollout_logprob_token_len_per_gpu="${MAX_ROLLOUT_LOGPROB_TOKEN_LEN_PER_GPU:-12288}"
max_ref_logprob_token_len_per_gpu="${MAX_REF_LOGPROB_TOKEN_LEN_PER_GPU:-12288}"

# vLLM: 80GB — higher KV/batch caps than 40GB; OOM on ``update_weights`` → lower ``VLLM_GPU_MEM_UTIL`` before batch size.
vllm_gpu_mem_util="${VLLM_GPU_MEM_UTIL:-0.76}"
