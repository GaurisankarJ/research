#!/usr/bin/env bash
# Qwen3 ReSearch GRPO (instruct checkpoint). Requires: pip install -e . (see verl_latest/setup.py).
# Print merged Hydra config only (no training, no wandb):
#   cd verl_latest && ./train_instruct.sh --hydra-cfg-job
...
ACTOR_MODEL_PATH=../models/Qwen3-0.6B
REWARD_MANAGER=re_search
ROLLOUT_N=5
SEARCH_URL=127.0.0.1:3005
PROJECT_NAME=research
EXPERIMENT_NAME=qwen3-0.6b-instruct
NNODES=1
N_GPUS_PER_NODE=1
# vLLM TP must be <= visible GPUs for rollout; use 2+ only with matching N_GPUS_PER_NODE.
TENSOR_MODEL_PARALLEL_SIZE=1
SAVE_FREQ=10
TEST_FREQ=10
TOTAL_EPOCHS=2
WANDB_API_KEY=${WANDB_API_KEY:-None}
SAVE_PATH=./results/qwen3-0.6b-instruct
