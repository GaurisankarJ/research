...
def tool_payload_valid(payload: str) -> tuple[bool, str, str]:
    ...
    if isinstance(args, str):
        ...
        return False, "tool_arguments_not_string_or_object", ""
    if not query:
...
def validate_reasoning_search_format(text: str) -> tuple[bool, str]:
    ...
    if "\\boxed{" not in answer_text or "}" not in answer_text:
...
def score_trace(text: str) -> TraceScore:
    ...
    answer_has_boxed = 1.0 if "\\boxed{" in answer_text else 0.0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AutoResearch prompt logging and scoring helper")
